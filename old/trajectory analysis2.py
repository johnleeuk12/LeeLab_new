...
for tr in np.arange(trmax):
    ...
    for n in good_list: 
        ...
        for ind in [0,1]:
            ...
            # max1 = np.max(np.mean(Y[X[:,0]==1],0))
            # max2 = np.max(np.mean(Y[X[:,1]==1],0))
            D[ind,tr][m,:] = D[ind,tr][m,:]/((np.max(np.mean(Y,0))) + alpha) # for original trajectories with Go+ NG
            # D[ind,tr][m,:] = D[ind,tr][m,:]/(np.max([max1,max2])  + alpha) # for original trajectories with Go+ NG
...
fig, axes = plt.subplots(1,1, figsize = (10,10))
for tr in [8,4,9]:
    axes.plot(np.mean(D[1,tr][d_list,:],0))
...
spks = D_r[8]
# dataname= 'D:\GitHub\Kilosort-Wanglab\Analysis_postphy\C6_hit.mat'
# MATfile = loadmat(dataname)


# spks = ndimage.gaussian_filter(MATfile['C6'].T,[0,4])
# dataname= 'D:\GitHub\Kilosort-Wanglab\Analysis_postphy\C6_FA.mat'
# MATfile = loadmat(dataname)
# spks2 = ndimage.gaussian_filter(MATfile['C6'].T,[0,4])
# fit rastermap
...
# fig, axs = plt.subplots(nrows=1, ncols=2, figsize=(30, 20))
# im1 = axs[0].imshow(zscore(spks[isort, :],axis = 1),clim = clim, cmap="viridis",  vmin=clim[0], vmax=clim[1], aspect="auto")
# axs[1].imshow(zscore(spks2[isort, :],axis = 1),clim = clim, cmap="viridis",  vmin=clim[0], vmax=clim[1], aspect="auto")
# fig.subplots_adjust(right=0.85)
# cbar_ax = fig.add_axes([0.88, 0.15, 0.04, 0.7])
# fig.colorbar(im1, cax=cbar_ax)
...
# axs[0].imshow(zscore(((D_r[0][isort, :])+(D_r[1][isort, :])+(D_r[2][isort, :])/3),axis = 1),clim = clim, cmap="gray_r",  vmin=clim[0], vmax=clim[1], aspect="auto")
# axs[1].imshow(zscore(((D_r[5][isort, :])+(D_r[6][isort, :])+(D_r[7][isort, :])/3),axis = 1),clim = clim, cmap="gray_r",  vmin=clim[0], vmax=clim[1], aspect="auto")
...
# im1 = axs[0].imshow(zscore(D_r[8][isort, :],axis = 1),clim = clim, cmap="hot_r",  vmin=clim[0], vmax=clim[1], aspect="auto")
# axs[1].imshow(zscore(D_r[9][isort, :]),clim = clim, cmap="gray_r",  vmin=clim[0], vmax=clim[1], aspect="auto")
...
pca = {}
max_k = 20;
# d_list = np.logical_and(good_list > 194, good_list < 700)
d_list = good_list > 194

# d_list3 = good_list <= 160
d_list3 = good_list <= 180

trmax = 8

d_list2 = d_list3
# d_list2 = good_list>-1
fig, axs = plt.subplots(trmax+2,6,figsize = (10,20))
...
# 
    ...
    test = pca[g].fit_transform(ndimage.gaussian_filter(R[g],[2,0]))        
    test = test.T
    for t in range(0,5):
...
# %% PCA and trajectory space with grouped units 11/25
...
# d_list = (good_list > 195)
d_list = np.logical_and(good_list > 195, good_list < 700)
...
def traj_dist(array):
    array = array[:,[0,1,3]]
    distance  =np.zeros((1,160))
    for t in np.arange(159):
        distance[0,t] = np.linalg.norm(array[t+1,:]-array[t,:])
    return np.sum(distance)


g = [traj_dist(traj[0][trmax]),traj_dist(traj[0][trmax+1]),
     traj_dist(traj[0][3]),traj_dist(traj[1][3]),
     traj_dist(traj[1][trmax]),traj_dist(traj[1][trmax+1])]

g = g/np.max(g)
fig, axes= plt.subplots(1,1,figsize = (5,5))
axes.bar(np.arange(6),g)
